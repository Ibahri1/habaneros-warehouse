...
import {
  bulkAdjustWarehouseInventory, deleteWarehouseCategory, deleteWarehouseLocation, deleteWarehouseProduct, deleteWarehouseUser,
...
export function WarehouseApp() {
  ...
  const adminNav:[View,string][]=[["dashboard","Dashboard"],["orders","Order Queue"],["orderHistory","Order History"],["products","Products"],["categories","Categories"],["locations","Locations"],["adjustment","Adjust"],["movements","Movement Log"],["users","Employees & Codes"],["settings","Settings"]];
...
function Admin(p:any){
  ...
  else if(p.view==="orders"){const orders=p.data.orders.filter((o:Order)=>!o.queue_hidden&&(o.status!=="Delivered"||!o.delivered_at||new Date(o.delivered_at).getTime()>=historyCutoff)).filter((o:Order)=>`${o.order_number} ${o.manager} ${o.location} ${o.status}`.toLowerCase().includes(queueSearch.toLowerCase()));const pickableOrders=orders.filter((o:Order)=>o.status!=="Delivered"&&o.status!=="Cancelled");content=<><div className="queue-screen no-print"><PageHead eyebrow="FULFILLMENT" title={p.data.user.role==="fulfillment"?"Orders":"Order queue"} subtitle="Delivered orders remain here for 30 days. Delivered or Cancelled orders can be moved to history sooner." action={<button className="secondary" onClick={()=>window.print()}>Print picking list</button>}/><div className="filters"><div className="search">⌕ <input placeholder="Search order, location, manager..." value={queueSearch} onChange={e=>setQueueSearch(e.target.value)}/></div></div>{selectedQueueOrders.length>0&&<div className="bulk-bar"><span><b>{selectedQueueOrders.length}</b> selected</span><button className="danger-button" onClick={()=>hideFromQueue(selectedQueueOrders)}>Remove selected Delivered/Cancelled orders from queue</button><button className="secondary" onClick={()=>setSelectedQueueOrders([])}>Clear</button></div>}<OrderList orders={orders} select={p.setSelectedOrder} queue selected={selectedQueueOrders} toggle={toggleQueueOrder} hide={id=>hideFromQueue([id])}/></div><PickingList orders={pickableOrders} products={p.data.products}/></>}
...
function OrderDetail({order,back,manager,update}:{order:Order;back:()=>void;manager?:boolean;update?:(s:string,f:string,d:string)=>void}){
  ...
  return <div className="print-area"><button className="back no-print" onClick={back}>← Back to orders</button><PageHead eyebrow={order.order_number} title={`${order.location} order`} subtitle={`Submitted by ${order.manager} · ${when(order.submitted_at)}`} action={<span className={statusClass(order.status)}>{order.status}</span>}/><div className="split"><section className="panel"><h3>Picking & packing details</h3>{order.items.map((x,i)=>{const picked=pickedItems.includes(x.id);return <div className={`pick-row ${picked?"is-picked":""}`} key={x.id}>{!manager&&<label className="pick-check no-print"><input type="checkbox" checked={picked} onChange={()=>togglePicked(x.id)}/><span>Picked</span></label>}<span className="pick-num">{i+1}</span><div className="grow"><b>{x.name}</b><small>{x.sku} · {x.unit_size}</small>{!manager&&<span className="item-location">Item Location: {x.item_location||"Not set"}</span>}</div><strong>{x.requested_quantity} requested</strong></div>})}{!manager&&<div className="screen-notes"><label>Fulfillment notes<textarea value={fulfillment} onChange={e=>setFulfillment(e.target.value)} placeholder="Substitutions, shortages, or packing notes..."/></label><label>Delivery note<textarea value={delivery} onChange={e=>setDelivery(e.target.value)} placeholder="Where and to whom the order was delivered..."/></label></div>}<div className="print-only print-notes"><h3>Order notes</h3><div><b>Manager note</b><p>{order.order_note||"None"}</p></div><div><b>Fulfillment note</b><p>{fulfillment||"None"}</p></div><div><b>Delivery note</b><p>{delivery||"None"}</p></div></div></section><section className="panel order-side"><h3>Order details</h3><dl><div><dt>Destination</dt><dd>{order.location}</dd></div><div><dt>Manager</dt><dd>{order.manager}</dd></div><div><dt>Items</dt><dd>{order.items.length}</dd></div><div><dt>Order note</dt><dd>{order.order_note||"None"}</dd></div></dl>{!manager&&<><label>Update status</label><div className="status-actions">{["Confirmed","Picking","Out for Delivery","Delivered","Cancelled"].map(status=><button key={status} className={order.status===status?"primary":"secondary"} onClick={()=>update?.(status,fulfillment,delivery)}>{status}</button>)}</div><button className="secondary wide" onClick={()=>update?.(order.status,fulfillment,delivery)}>Save notes</button><button className="print-button" onClick={()=>window.print()}>Print picking list</button></>}</section></div></div>;
}

function InventoryForm({data,act}:{data:AppData;act:any}){
  const [selected,setSelected]=useState<string[]>([]);const [quantity,setQuantity]=useState("");const [reason,setReason]=useState("Inventory adjustment");const [search,setSearch]=useState("");
  const products=data.products.filter(x=>`${x.name} ${x.sku||""}`.toLowerCase().includes(search.toLowerCase()));
  const toggle=(id:string)=>setSelected(current=>current.includes(id)?current.filter(x=>x!==id):[...current,id]);
  async function submit(){const amount=Number(quantity);if(!selected.length||!amount||!reason.trim())return;if(selected.length>1&&!window.confirm(`Apply ${amount>0?"+":""}${amount} to ${selected.length} selected products?`))return;try{await act(()=>bulkAdjustWarehouseInventory(selected,amount,reason),`${selected.length} product${selected.length===1?"":"s"} adjusted`);setQuantity("");setSelected([])}catch{return}}
  return <><PageHead eyebrow="INVENTORY" title="Adjust inventory" subtitle="Apply one quantity change and reason to one or several products."/><section className="panel form-panel bulk-adjustment"><div className="selection-head"><label className="grow">Select products<div className="search">⌕ <input aria-label="Search adjustment products" placeholder="Search products or SKU" value={search} onChange={e=>setSearch(e.target.value)}/></div></label><button className="secondary" onClick={()=>setSelected(selected.length===data.products.length?[]:data.products.map(x=>x.id))}>{selected.length===data.products.length?"Clear all":"Select all"}</button></div><div className="product-checklist">{products.map(product=><label className="product-check" key={product.id}><input type="checkbox" checked={selected.includes(product.id)} onChange={()=>toggle(product.id)}/><span><b>{product.name}</b><small>{product.sku||"No SKU"}</small></span><span className="stock-summary">On hand <b>{product.on_hand}</b><small>{product.available} available</small></span></label>)}{!products.length&&<p className="empty-copy">No matching products.</p>}</div><div className="selection-count"><b>{selected.length}</b> product{selected.length===1?"":"s"} selected</div><div className="form-grid"><label>Quantity change for each<input type="number" value={quantity} onChange={e=>setQuantity(e.target.value)} placeholder="e.g. 10 or -2"/></label><label>Adjustment reason<input value={reason} onChange={e=>setReason(e.target.value)} placeholder="Required reason"/></label></div><button className="primary" disabled={!selected.length||!Number(quantity)||!reason.trim()} onClick={submit}>Apply adjustment to {selected.length||0} product{selected.length===1?"":"s"}</button></section></>;
}
