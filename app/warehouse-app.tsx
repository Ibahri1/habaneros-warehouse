...
type Order = {id:string;order_number:string;manager_id:string|null;manager:string;location_id:string|null;location:string;status:string;order_note:string|null;fulfillment_note:string|null;delivery_note:string|null;submitted_at:string;delivered_at:string|null;items:OrderItem[]};
...
export function WarehouseApp() {
  ...
  async function updateOrder(order:Order,status:string,fulfillmentNote:string,deliveryNote:string){try{await act(()=>updateWarehouseOrder(order.id,status,fulfillmentNote,deliveryNote),`Order marked ${status}`)}catch{return}}

  if(loading)return <main className="login-page"><section className="login-card loading-card"><span className="pepper large">H</span><h2>Opening warehouse...</h2></section></main>;
  ...
  return <div className={`app-shell ${working?"is-working":""}`}>
    <header className="topbar"><button className="brand" onClick={()=>navigate(role==="manager"?"catalog":"dashboard")}><span className="pepper">H</span><span>HABANEROS<small>WAREHOUSE</small></span></button><div className="account"><span className="account-copy"><b>{data.user.display_name}</b><small>{role==="manager"?"Store Manager":role==="admin"?"Administrator":"Fulfillment"}</small></span><button className="icon-button" onClick={logout} aria-label="Sign out">↪</button></div></header>
...
function Admin(p:any){
  ...
  else if(p.view==="orders"){const orders=p.data.orders.filter((o:Order)=>o.status!=="Delivered"||!o.delivered_at||new Date(o.delivered_at).getTime()>=historyCutoff).filter((o:Order)=>`${o.order_number} ${o.manager} ${o.location} ${o.status}`.toLowerCase().includes(queueSearch.toLowerCase()));content=<><PageHead eyebrow="FULFILLMENT" title="Order queue" subtitle="Delivered orders remain here for 30 days." action={<button className="secondary" onClick={()=>window.print()}>Print picking list</button>}/><div className="filters"><div className="search">⌕ <input placeholder="Search order, location, manager..." value={queueSearch} onChange={e=>setQueueSearch(e.target.value)}/></div></div><OrderList orders={orders} select={p.setSelectedOrder}/></>}
  else if(p.view==="orderHistory"){const orders=p.data.orders.filter((o:Order)=>o.status==="Delivered"&&!!o.delivered_at&&new Date(o.delivered_at).getTime()<historyCutoff).filter((o:Order)=>`${o.order_number} ${o.manager} ${o.location}`.toLowerCase().includes(queueSearch.toLowerCase()));content=<><PageHead eyebrow="FULFILLMENT" title="Order history" subtitle="Delivered orders older than 30 days."/><div className="filters"><div className="search">⌕ <input placeholder="Search order, location, manager..." value={queueSearch} onChange={e=>setQueueSearch(e.target.value)}/></div></div><OrderList orders={orders} select={p.setSelectedOrder}/></>}
...
function OrderList({orders,select}:{orders:Order[];select:(o:Order)=>void}){return <section className="panel order-list"><OrderRows orders={orders} select={select}/>{!orders.length&&<p className="empty-copy">No orders found.</p>}</section>}
function OrderRows({orders,select}:{orders:Order[];select:(o:Order)=>void}){return <div className="orders">{orders.map(o=><button className="order-row" key={o.id} onClick={()=>select(o)}><span><b>{o.order_number}</b><small>{o.location}</small></span><span><b>{o.manager}</b><small>{when(o.submitted_at)}</small></span><span>{o.items.length} items</span><span className={statusClass(o.status)}>{o.status}</span><i>→</i></button>)}</div>}
