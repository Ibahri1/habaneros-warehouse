...
export async function getWarehouseData() {
  ...
  if(data?.user?.role!=="manager"){
    ...
    const progress=await rpc<{id:string;picked_at:string|null;picked_by:string|null}[]>("warehouse_get_picking_progress");
    const byItem=new Map((progress||[]).map(item=>[item.id,item]));
    data.orders=(data.orders||[]).map((order:any)=>({...order,queue_hidden:hiddenIds.has(order.id),items:(order.items||[]).map((item:any)=>({...item,...byItem.get(item.id)}))}));
  }
  return data;
}
...
export const setWarehouseOrderItemPicked = (orderId:string,itemId:string,picked:boolean,expectedPickedAt:string|null) =>
  rpc<{id:string;picked_at:string|null;picked_by:string|null}>("warehouse_set_order_item_picked",{input_order_id:orderId,input_item_id:itemId,input_picked:picked,input_expected_picked_at:expectedPickedAt});
export const completeWarehousePickedOrder = (orderId:string,fulfillmentNote:string,deliveryNote:string) =>
  rpc<string>("warehouse_complete_picked_order",{input_order_id:orderId,input_fulfillment_note:fulfillmentNote,input_delivery_note:deliveryNote});
